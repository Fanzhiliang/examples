	...
	var i,len,
		...
		globalPage={
			...
			menus : (function(document){
				var menuItem = Array.prototype.slice.call($(".menu-item")),
					upLoad = Array.prototype.slice.call($(".up-load"));
					items = menuItem.concat(upLoad),
					menus = [],i,len,
					displayNav = function(){
						addClassName(this.nav,"display");
						deleteClassName(this.nav,"conceal");
					},
					hiddenNav = function(){
						addClassName(this.nav,"conceal");
						deleteClassName(this.nav,"display");
					},
					setMenuDrop = function(item){
						var navs = [],win={},doc={};
						if(item.className.indexOf("menu-item")!=-1){
							navs = item.getElementsByClassName("i-frame");
						}else if(item.className.indexOf("up-load")!=-1){
							navs = item.getElementsByClassName("up-nav");
						}
						if(navs.length<1){
							item.nav=false;
						}else{
							item.nav = navs[0];
							item.displayNav = displayNav;
							item.hiddenNav = hiddenNav;
							item.hover(displayNav,hiddenNav);
						}
						return item;
					};
				for(i=0,len=items.length;i<len;++i){
					menus.push(setMenuDrop(items[i]));
				}
				return menus;
			})(document)
		},
		Slider = function(scrollx,speed,interval,cycleInterval){
			var cycleThreadsID = (function(len){return len;})(globalPage.cycleThreads.length),
				...
				trig = setItems(scrollx.getElementsByClassName("trig"),"span",false),
